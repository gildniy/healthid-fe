...
describe('Render Supplier Form component', () => {
  ...
  it('renders without crashing', () => {
    ...
    phoneInput.props().onChange({});
    expect(wrapper.find('ImageUpload').length).toBe(1);
  });

  it('renders on edit without crashing', () => {
    const newProps = props;
    newProps.type = 'edit';
    newProps.state.paymentTerms = 'ON_CREDIT',
    newProps.state.creditDays = 12
    newProps.initialData.countries = '';
    newProps.state.cities = [];
    const newWrapper = shallow(<SupplierForm {...newProps} />);
